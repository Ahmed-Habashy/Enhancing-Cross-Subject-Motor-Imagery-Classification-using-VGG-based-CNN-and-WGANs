...
def get_concat_v(im1, im2):
    dst = Image.new('L', (im1.width, im1.height + im2.height ))  #'RGB'
    dst.paste(im1, (0, 0))
    ...
    for i in range(num_trials):
        for j in chan_ind:   
            ...
            if draw==True:
                ...
                plt.pcolormesh(t, f, np.abs(Zxx),   cmap='gray', shading='gouraud')  # cmap = 'jet'
                plt.xlim(0,4)
                plt.ylim(8,30)
                plt.axis('off')
                makedirs('D:\PhD Ain Shams\Dr Seif\GANs\python_ex\BCI_IV_1\spectrogram_ch3,4\gray\CL{0}_3ch '.format(cl ), exist_ok=True) 
                plt.savefig('D:\PhD Ain Shams\Dr Seif\GANs\python_ex\BCI_IV_1\spectrogram_ch3,4\gray\CL{0}_3ch\cl{0} STFT_{3}_ch {1}_t{2} .png'.format(cl ,j, i, sub), bbox_inches= 'tight', pad_inches= 0)
                plt.show() 
        img26 = Image.open('D:\PhD Ain Shams\Dr Seif\GANs\python_ex\BCI_IV_1\spectrogram_ch3,4\gray\CL{0}_3ch\cl{0} STFT_{2}_ch 26_t{1} .png' .format(cl , i, sub)) # Path to image
        img30 = Image.open('D:\PhD Ain Shams\Dr Seif\GANs\python_ex\BCI_IV_1\spectrogram_ch3,4\gray\CL{0}_3ch\cl{0} STFT_{2}_ch 30_t{1} .png' .format(cl , i, sub)) # Path to image
        get_concat_v(img26,  img30).save('D:\PhD Ain Shams\Dr Seif\GANs\python_ex\BCI_IV_1\spectrogram_ch3,4\gray\sub_{2}\CL{0}\cl{0} ch3_{2}_tr{1}.bmp' .format(cl , i, sub)) 
