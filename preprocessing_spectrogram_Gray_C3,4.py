...
dataset1 = scipy.io.loadmat('\BCICIV_calib_ds1{}.mat'.format(sub), struct_as_record=True)
print(dataset1.keys())
    ...
    for i in range(num_trials):
        for j in chan_ind:   
            ...
            if draw==True:
                ...
                plt.ylim(8,30)
                plt.axis('off')
                makedirs('\gray\CL{0}_3ch '.format(cl ), exist_ok=True) 
                plt.savefig('\gray\CL{0}_3ch\cl{0} STFT_{3}_ch {1}_t{2} .png'.format(cl ,j, i, sub), bbox_inches= 'tight', pad_inches= 0)
                plt.show() 
        img26 = Image.open('\gray\CL{0}_3ch\cl{0} STFT_{2}_ch 26_t{1} .png' .format(cl , i, sub)) # Path to image
        img30 = Image.open('\gray\CL{0}_3ch\cl{0} STFT_{2}_ch 30_t{1} .png' .format(cl , i, sub)) # Path to image
        get_concat_v(img26,  img30).save('\gray\sub_{2}\CL{0}\cl{0} ch3_{2}_tr{1}.bmp' .format(cl , i, sub)) 
